...
namespace DAL
{
    public class UserInfo
    {
        ...
        public Model.UserInfo GetModel(string UserNmae)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from userinfo where username=@username";
                return cn.QueryFirstOrDefault<Model.UserInfo>(sql, new { username = UserNmae });
            }
        }
        ...
        public int Add(Model.UserInfo user)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into userinfo values(@userNmae,@passWord,@@qq,@email,@type,@userImg);";
        ...
        public int Delete(string username)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "detele from userinfo where username=@username";
